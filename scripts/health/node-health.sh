...
PROJECT_DIR="${1:?Usage: node-health.sh <project-path>}"
...
echo "Node Project Health: $(basename "$PROJECT_DIR")"
echo "========================"

check "Directory exists"       test -d "$PROJECT_DIR"
check "package.json exists"    test -f "$PROJECT_DIR/package.json"
check "node_modules installed" test -d "$PROJECT_DIR/node_modules"
